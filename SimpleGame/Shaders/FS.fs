...
void TextureQ2()
{
	float tx = fract(v_TPos.x * 3);	// 0~1 * 3
	float ty = v_TPos.y / 3;		// 0~0.3
...
void NumsAI()
{
    ...
    // 2. v_TPos.x를 자릿수만큼 분할하여, 현재 픽셀이 어느 자리(왼쪽부터 0번째)에 있는지 계산
    float scaledX = v_TPos.x * float(digitCount);   // 0 ~ digitCount
    int digitIndex = int(floor(scaledX));           // 왼쪽 자릿수부터 0, 1, 2, ...
    if (digitIndex >= digitCount) digitIndex = digitCount - 1; // 경계 보정
    float localX = fract(scaledX);                  // 셀 내부 0~1

    // 3. 해당 자리의 숫자 값 추출
    //    ex) 1234에서 왼쪽(digitIndex=0)은 1000자리 → power = 10^(digitCount-1-digitIndex)
    int power = 1;
    ...
    // 4. 기존 Nums 로직: 0~9 텍스처에서 해당 숫자 셀을 샘플링
    //    가로 5칸, 세로 2칸 그리드. 가로 방향만 localX로 교체.
    float tx = localX / 5.0;
